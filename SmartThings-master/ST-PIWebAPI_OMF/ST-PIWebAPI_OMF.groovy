 ...
 *
 *  Copyright 2020 Michael Horrocks
 *
...
def evtHandler(evt) {
	...
	// container will be the <device name>.<event name> - for example, "kitchen temperature.temperature"
	// type will be the <event name>, which maps directly to a capability - for example, "temperature"
	def event_name = "${evt.name}"

	def device_name = evt.getDevice().getName()

	def container = "$device_name"+"."+"$event_name"
	def timestamp = "${evt.isoDate}"
	def value = evt.value

	log.debug "$container : $timestamp : $value"

	// need to translate 

	omfWriter(container, capability, timestamp, value)
}

def omfWriter(container, capability, timestamp, value) {

	def omf = new groovy.json.JsonOutput().toJson(
